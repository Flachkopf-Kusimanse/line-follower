 ...
 {    
      ...
      else if(digitalRead(s4)==HIGH && digitalRead(s5)==HIGH && digitalRead(s6)==LOW && digitalRead(s7)==HIGH&& online==true) //TILT RIGHT
      {
        tiltright;
            ...
            else if(digitalRead(s4)==LOW && digitalRead(s5)==HIGH && digitalRead(s6)==HIGH && digitalRead(s7)==HIGH&& online==true) //TILT SPECIAL RIGHT
      {
                  digitalWrite(MotorAip1,HIGH);
          digitalWrite(MotorAip2,LOW);
          digitalWrite(MotorBip1,LOW);
          digitalWrite(MotorBip2,LOW);
          analogWrite (enA, veltsl);
          analogWrite (enB, veltsh);
      }
      else if(digitalRead(s4)==HIGH && digitalRead(s5)==HIGH && digitalRead(s6)==HIGH && digitalRead(s7)==LOW && online==true) //TILT SPECIAL RIGHT
      {
                  digitalWrite(MotorAip1,HIGH);
          digitalWrite(MotorAip2,LOW);
          digitalWrite(MotorBip1,LOW);
